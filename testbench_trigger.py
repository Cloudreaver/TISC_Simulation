...
if __name__ == '__main__':
   ...
   if(boresight==0):
      if(baseline==0):
         output_dir = str(os.path.dirname(os.path.realpath(__file__))+"/testbench_analysis/output/on_boresight_long_baseline_TISC_testbench_"+time.strftime('%Y_%m_%d_%H.%M.%S'))
      if(baseline==1):
         output_dir = str(os.path.dirname(os.path.realpath(__file__))+"/testbench_analysis/output/on_boresight_short_baseline_TISC_testbench_"+time.strftime('%Y_%m_%d_%H.%M.%S'))
   if(boresight==1):
      if(baseline==0):
         output_dir = str(os.path.dirname(os.path.realpath(__file__))+"/testbench_analysis/output/off_boresight_long_baseline_TISC_testbench_"+time.strftime('%Y_%m_%d_%H.%M.%S'))
      if(baseline==1):
         output_dir = str(os.path.dirname(os.path.realpath(__file__))+"/testbench_analysis/output/off_boresight_short_baseline_TISC_testbench_"+time.strftime('%Y_%m_%d_%H.%M.%S'))
  ...
  # Set some variables
   ...
   for SNR_counter in range(len(SNR)):
      ...
      if(debug==True): print "DEF SNR: %1.2f"%ghi_SNR_value
      #time,a_uncor,b_uncor,c_uncor,d1,d2,d3,a,b,c,d4,d5,d6 = ttb.read_data_from_data_file(abc_atten_value,samples_to_read,0)
      ...
      if(SNR[SNR_counter]!=0):
         if(debug): print "Reading in impulsive data"
         ...
         if(abc_atten_value!=def_atten_value):
            time,d_uncor,e_uncor,f_uncor,g_uncor,h_uncor,i_uncor,d,e,f,g,h,i = ttb.read_data_from_data_file(def_atten_value,samples_to_read,1)
         if(ghi_atten_value!=def_atten_value):
            time,g_uncor,h_uncor,i_uncor,d1,d2,d3,g,h,i,d4,d5,d6 = ttb.read_data_from_data_file(def_atten_value,samples_to_read,2)
            
         if(debug): print "Delaying waveforms"
      ...
      #time,a_uncor_therm,b_uncor_therm,c_uncor_therm,d1,d2,d3,a_therm,b_therm,c_therm,d4,d5,d6 = ttb.read_data_from_data_file(100.00,samples_to_read*thermal_noise_factor,0)
      #print len(time)
      #time,d_uncor_therm,e_uncor_therm,f_uncor_therm,d1,d2,d3,d_therm,e_therm,f_therm,d4,d5,d6 = ttb.read_data_from_data_file(100.00,samples_to_read*thermal_noise_factor,1)
      #print len(time)
      ...
      time,g_uncor_therm,h_uncor_therm,i_uncor_therm,d1,d2,d3,g_therm,h_therm,i_therm,d4,d5,d6 = ttb.read_data_from_data_file(100.00,samples_to_read*thermal_noise_factor,1)
      ...
      for timestep in range(0,num_runs):
         ...
         if ((timestep % int((simulation_rate/event_rate)) != 0 or SNR[SNR_counter]==0)):
            ...
            d1,abc_max_sum,as_abc_max_sum,abc_correlation_mean,d4,d5,as_abc_angle,abc_angle,d8,d9 = sum_correlate(num_samples_per_window,a_therm[start:end],b_therm[start:end],c_therm[start:end],threshold,abc_baseline,TISC_sample_length=16,delay_type_flag=1,average_subtract_flag=1,correlation_mean=abc_correlation_mean)
            d1,def_max_sum,as_def_max_sum,def_correlation_mean,d4,d5,as_def_angle,def_angle,d8,d9 = sum_correlate(num_samples_per_window,d_therm[start:end],e_therm[start:end],f_therm[start:end],threshold,def_baseline,TISC_sample_length=16,delay_type_flag=1,average_subtract_flag=1,correlation_mean=def_correlation_mean)
            d1,ghi_max_sum,as_ghi_max_sum,ghi_correlation_mean,d4,d5,as_ghi_angle,ghi_angle,d8,d9 = sum_correlate(num_samples_per_window,g_therm[start:end],h_therm[start:end],i_therm[start:end],threshold,ghi_baseline,TISC_sample_length=16,delay_type_flag=1,average_subtract_flag=1,correlation_mean=ghi_correlation_mean)
            ...
            d1,abc_max_sum_uncor,as_abc_max_sum_uncor,abc_correlation_mean_uncor,d4,d5,as_abc_angle_uncor,abc_angle_uncor,d8,d9 = sum_correlate(num_samples_per_window,a_uncor_therm[start:end],b_uncor_therm[start:end],c_uncor_therm[start:end],threshold,abc_baseline,TISC_sample_length=16,delay_type_flag=1,average_subtract_flag=1,correlation_mean=abc_correlation_mean_uncor)
            d1,def_max_sum_uncor,as_def_max_sum_uncor,def_correlation_mean_uncor,d4,d5,as_def_angle_uncor,def_angle_uncor,d8,d9 = sum_correlate(num_samples_per_window,d_uncor_therm[start:end],e_uncor_therm[start:end],f_uncor_therm[start:end],threshold,def_baseline,TISC_sample_length=16,delay_type_flag=1,average_subtract_flag=1,correlation_mean=def_correlation_mean_uncor)
            ...
            def_hist.append(def_max_sum)
            ghi_hist.append(ghi_max_sum)
            d1,abc_max_sum_uncor,as_abc_max_sum_uncor,abc_correlation_mean_uncor,d4,d5,as_abc_angle_uncor,abc_angle_uncor,d8,d9 = sum_correlate(num_samples_per_window,a_uncor[start:end],b_uncor[start:end],c_uncor[start:end],threshold,abc_baseline,TISC_sample_length=16,delay_type_flag=1,average_subtract_flag=1,correlation_mean=abc_correlation_mean_uncor)
            d1,def_max_sum_uncor,as_def_max_sum_uncor,def_correlation_mean_uncor,d4,d5,as_def_angle_uncor,def_angle_uncor,d8,d9 = sum_correlate(num_samples_per_window,d_uncor[start:end],e_uncor[start:end],f_uncor[start:end],threshold,def_baseline,TISC_sample_length=16,delay_type_flag=1,average_subtract_flag=1,correlation_mean=def_correlation_mean_uncor)
            d1,ghi_max_sum_uncor,as_ghi_max_sum_uncor,ghi_correlation_mean_uncor,d4,d5,as_ghi_angle_uncor,ghi_angle_uncor,d8,d9 = sum_correlate(num_samples_per_window,g_uncor[start:end],h_uncor[start:end],i_uncor[start:end],threshold,ghi_baseline,TISC_sample_length=16,delay_type_flag=1,average_subtract_flag=1,correlation_mean=ghi_correlation_mean_uncor)
         ...
         """
         data_filename.write('\n'+str(center_SNR_value)+','+str(abc_max_sum)+','+str(as_abc_max_sum)+','+str(def_max_sum)+','+str(as_def_max_sum)+','+str(ghi_max_sum)+','+str(as_ghi_max_sum))
         ...
         for threshold_counter in range(0,len(threshold)):
            ...
            if((as_total_correlated_sum+max_offset)>threshold[threshold_counter]):
               num_passed_as_events[threshold_counter] += 1
               #print num_passed_events[threshold_counter]
            if((total_correlated_sum)>threshold[threshold_counter]):
               num_passed_events[threshold_counter] += 1
               #print num_passed_events[threshold_counter]
            if((as_total_correlated_sum_uncor+max_offset)>threshold[threshold_counter]):
               num_passed_as_events_uncor[threshold_counter] += 1
               #print num_passed_events[threshold_counter]
            if((total_correlated_sum_uncor)>threshold[threshold_counter]):
   ...
   if(boresight==0):
      if(baseline==0):
         plt.figure(1)
         plt.title("On Boresight, Long Baseline, System Rate: %1.2fMHz, Event Rate: %1.2fMHz" % (simulation_rate/10**6,event_rate/10**6))
         plt.figure(2)
         plt.title("Uncorrected On Boresight, Long Baseline, System Rate: %1.2fMHz, Event Rate: %1.2fMHz" % (simulation_rate/10**6,event_rate/10**6))
      elif(baseline==1):
         plt.figure(1)
         plt.title("On Boresight, Short Baseline, System Rate: %1.2fMHz, Event Rate: %1.2fMHz" % (simulation_rate/10**6,event_rate/10**6))
         plt.figure(1)
         plt.title("Uncorrected On Boresight, Short Baseline, System Rate: %1.2fMHz, Event Rate: %1.2fMHz" % (simulation_rate/10**6,event_rate/10**6))
   if(boresight==1):
      if(baseline==0):
         plt.figure(1)
         plt.title("Off Boresight, Long Baseline, Symvstem Rate: %1.2fMHz, Event Rate: %1.2fMHz" % (simulation_rate/10**6,event_rate/10**6))
         plt.figure(2)
         plt.title("Uncorrected Off Boresight, Long Baseline, System Rate: %1.2fMHz, Event Rate: %1.2fMHz" % (simulation_rate/10**6,event_rate/10**6))
      elif(baseline==1):
         plt.figure(1)
         plt.title("Off Boresight, Short Baseline, System Rate: %1.2fMHz, Event Rate: %1.2fMHz" % (simulation_rate/10**6,event_rate/10**6))
         plt.figure(2)
         plt.title("Uncorrected, Off Boresight, Short Baseline, System Rate: %1.2fMHz, Event Rate: %1.2fMHz" % (simulation_rate/10**6,event_rate/10**6))
   plt.figure(1)
